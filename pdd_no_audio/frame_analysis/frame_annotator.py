# pdd_no_audio/frame_analysis/frame_annotator.py
...
from pdd_no_audio.config import annotation_config
...
def annotate_frame(
    ...
    enabled = enabled if enabled is not None else annotation_config.enabled
...
def _draw_region_highlight(
    ...
    box_color = annotation_config.box_color
    ...
    cv2.circle(frame, (circle_x, circle_y), 18, box_color, -1)
    cv2.putText(
        frame, str(step_number),
        (circle_x - 8, circle_y + 6),
        cv2.FONT_HERSHEY_SIMPLEX, 0.6,
        annotation_config.label_text_color, 2
    )
    ...
    cv2.arrowedLine(
        ...
        annotation_config.arrow_color, 2,
        tipLength=0.15
    )
    ...
    if label:
        ...
        (tw, th), _ = cv2.getTextSize(
            ...
            annotation_config.font_scale * 0.6, 1
        )
        cv2.rectangle(
            frame,
            (x, label_y - th - 5),
            (x + tw + 10, label_y + 5),
            annotation_config.label_bg_color, -1
        )
        cv2.putText(
            frame, label_short,
            (x + 5, label_y),
            cv2.FONT_HERSHEY_SIMPLEX,
            annotation_config.font_scale * 0.6,
            annotation_config.label_text_color, 1
        )
...
def _draw_step_label(
    ...
    cv2.putText(
        ...
        (10, 28),
        cv2.FONT_HERSHEY_SIMPLEX,
        annotation_config.font_scale,
        annotation_config.label_text_color,
        annotation_config.font_thickness
    )
